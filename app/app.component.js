var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
var core_1 = require('@angular/core');
var camper_service_1 = require("./camper.service");
var AppComponent = (function () {
    function AppComponent(camperService) {
        this.camperService = camperService;
    }
    AppComponent.prototype.ngOnInit = function () {
        this.getCampers();
    };
    AppComponent.prototype.getCampers = function () {
        var _this = this;
        this.camperService
            .getCampers()
            .then(function (campers) { return _this.campers = campers; })
            .catch(function (error) { return _this.error = error; });
    };
    AppComponent = __decorate([
        core_1.Component({
            selector: 'my-app',
            template: "\n    <h1 class=\"text-center\">Free Code Camp Leadership Board</h1>\n    <table class=\"table table-striped\">\n    <thead>\n      <tr>\n        <th class=\"text-center\">#</th>\n        <th class=\"text-center\">Camper Name</th>\n        <th class=\"text-center\" (click)=\"sortByPointsInPast30Days()\">Points in past 30 days</th>\n        <th class=\"text-center\" (click)=\"sortByAllTimePoints()\">All time points</th>\n      </tr>\n    </thead>\n    <tbody>\n      <tr *ngFor=\"let camper of campers; let idx = index \">\n        <td class=\"text-center\">{{idx+1}}</td>\n        <td class=\"text-center\">{{camper.username}}</td>\n        <td class=\"text-center\">{{camper.recent}}</td>\n        <td class=\"text-center\">{{camper.alltime}}</td>\n      </tr>\n    </tbody>\n  </table>\n    ",
            providers: [camper_service_1.CamperService]
        }), 
        __metadata('design:paramtypes', [camper_service_1.CamperService])
    ], AppComponent);
    return AppComponent;
})();
exports.AppComponent = AppComponent;
//# sourceMappingURL=app.component.js.map